...
namespace Cfg.Net {
    ...
    public abstract class CfgNode {
        ...
        private static Dictionary<string, char> Entities => _entities ?? (_entities = new Dictionary<string, char>(StringComparer.Ordinal) {
            {"Aacute", "\x00c1"[0]},
            {"aacute", "\x00e1"[0]},
            {"Acirc", "\x00c2"[0]},
            {"acirc", "\x00e2"[0]},
        ...
        public T GetValidatedOf<T>(Action<T> setter = null) {
            ...
            if (!node.Errors().Any()) return typed;
        ...
        protected IEnumerable<string[]> LoadEnvironment(INode node, Dictionary<string, string> parameters) {
            for (int i = 0; i < node.SubNodes.Count; i++) {
                INode environmentsNode = node.SubNodes[i];
                ...
                if (environmentsNode.SubNodes.Count > 1) {
                    ...
                    for (int j = 0; j < environmentsNode.SubNodes.Count; j++) {
                        ...
                        string value = CheckParameters(parameters, defaultEnvironment.Value);
                ...
                INode parametersNode = environmentNode.SubNodes[0];
            ...
            for (int i = 0; i < node.SubNodes.Count; i++) {
                ...
                if (_metadata.ContainsKey(subNodeKey)) {
                    ...
                    if (item.SharedProperty != null) {
                        Dictionary<string, CfgMetadata> sharedMetadata = GetMetadata(item.ListType, Events, _builder);
                    ...
                    for (int j = 0; j < subNode.SubNodes.Count; j++) {
                        ...
                        if (add.Name.Equals("add", StringComparison.Ordinal)) {
                            string addKey = NormalizeName(_type, subNode.Name, _builder);
                            addHits.Add(addKey);
                            if (item.Loader == null) {
        ...
        private static string NormalizeName(Type type, string name, StringBuilder builder) {
            Dictionary<string, string> cache = _nameCache[type];
                ...
                if (_shorthand.MethodDataLookup.ContainsKey(expression.Method)) {
                    ...
                    if (methodData.Signature.Parameters.Count == 1 && expression.SingleParameter != string.Empty) {
                        ...
                        for (int i = 0; i < passedParameters.Length; i++) {
                            ...
                            if (split.Length == 2) {
                                string name = NormalizeName(typeof(Parameter), split[0], _builder);
                                shorthandNode.Attributes.Add(new ShorthandAttribute(name, split[1]));
                                signatureParameters.RemoveAll(
                                    p => NormalizeName(typeof(Parameter), p.Name, _builder) == name);
        ...
        private string CheckParameters(IDictionary<string, string> parameters, string input) {
            if (parameters == null || input.IndexOf('@') < 0)
                return input;
            Tuple<string, string[]> response = ReplaceParameters(input, parameters, _builder);
                    ...
                    if (index > 0 && input[index] == CfgConstants.ENTITY_END) {
                        ...
                        if (entity.Length > 1 && entity[0] == '#') {
                            ...
                            if (entity[1] == 'x' || entity[1] == 'X') {
                                parsedSuccessfully = UInt32.TryParse(entity.Substring(2), NumberStyles.AllowHexSpecifier,
                                    NumberFormatInfo.InvariantInfo, out parsedValue);
                            } else {
                                parsedSuccessfully = UInt32.TryParse(entity.Substring(1), NumberStyles.Integer,
                                    NumberFormatInfo.InvariantInfo, out parsedValue);
                            }
