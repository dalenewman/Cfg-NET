...
namespace Cfg.Net {
   ...
   public abstract class CfgNode {
      ...
      internal IActiveReader ActiveReader { get; set; }
      ...
      protected CfgNode(params IDependency[] dependencies) : this() {
         ...
         foreach (var dependency in dependencies.Where(dependency => dependency != null)) {
            ...
            } else if (dependency is IActiveReader) {
               ActiveReader = dependency as IActiveReader;
      ...
      public void Load(string cfg, IDictionary<string, string> parameters = null) {
         ...
         try {
            if (Reader != null) {
               ...
               cfg = ActiveReader.Read(cfg, parameters, Events.Logger);
               ActiveReader.Changed -= OnChange;
               ActiveReader.Changed += OnChange;
               if (Events.Logger.Errors().Any()) {
                  return;
               }
            ...
            var tip = string.IsNullOrEmpty(cfg) ? ' ' : cfg[0];
            if (Parser == null) {
               switch (tip) {
                  case '{':
                  ...
                  case '<':
                     ...
                     break;
                  default:
                     Events.InvalidConfiguration(tip);
                     return;
               }
            } else {
      ...
      /// <summary>
      /// This is triggered by an IActiveReader.  You may override it if 
      /// you need it to do more than just re-load the configuration.
      /// </summary>
      /// <param name="sender"></param>
      /// <param name="e"></param>
      protected virtual void OnChange(object sender, CfgEventArgs e) {
         Load(e.Source, e.Parameters);
      }
